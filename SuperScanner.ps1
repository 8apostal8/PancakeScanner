...
Write-Host "               🥞 PancakeScanner Ultimate PRO v3.9.1           " -ForegroundColor Cyan
Write-Host "===============================================================" -ForegroundColor Cyan
...
if ($ScanMode -eq "2") {
    Write-Host "`n Checking IFEO Injections..." -ForegroundColor Cyan; $IfeoPath = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options"
    Get-ChildItem -Path $IfeoPath -ErrorAction SilentlyContinue | ForEach-Object {
        $SubKey = $_.Name.Split("\")[-1]; if (Get-ItemProperty -Path "$IfeoPath\$SubKey" -Name "Debugger" -ErrorAction SilentlyContinue) {
            Write-Host "  [!] IFEO Hijack Found: $SubKey -> Active Debugger Trap!" -ForegroundColor Red; $TotalThreats++; $ThreatList += "Registry IFEO Hijack: $SubKey"
        }
    }
    ...
    $Shell = (Get-ItemProperty -Path $WinlogonPath -Name "Shell" -ErrorAction SilentlyContinue).Shell; $Userinit = (Get-ItemProperty -Path $WinlogonPath -Name "Userinit" -ErrorAction SilentlyContinue).Userinit
    if ($Shell -ne "explorer.exe" -and $Shell -ne $null) { Write-Host "  [!] Malicious Shell replacement detected: $Shell" -ForegroundColor Red; $TotalThreats++; $ThreatList += "Malicious Shell Modification: $Shell" }
    if ($Userinit -notlike "*userinit.exe*" -and $Userinit -ne $null) { Write-Host "  [!] Malicious Userinit modification detected: $Userinit" -ForegroundColor Red; $TotalThreats++; $ThreatList += "Malicious Userinit Modification: $Userinit" }
    ...
    Get-CimInstance -ClassName Win32_Service -ErrorAction SilentlyContinue | Where-Object { $_.PathName -like "*AppData*" -or $_.PathName -like "*Temp*" -or $_.PathName -like "*Users\Public*" } | ForEach-Object {
        Write-Host "  [!] Alert: Service '$($_.Name)' is running from temporary folders!" -ForegroundColor Yellow; $TotalThreats++; $ThreatList += "Suspicious AppData/Temp Service: $($_.Name)"
    }
    ...
    (Get-MpPreference -ErrorAction SilentlyContinue).ExclusionPath | ForEach-Object {
        Write-Host "  [!] Defender Exclusion Loophole Found: $_" -ForegroundColor Yellow; $TotalThreats++; $ThreatList += "Defender Exclusion Loophole: $_"
    }
}
Write-Host "`n Analyzing HOSTS file loops..." -ForegroundColor Cyan; $HostsPath = "$env:windir\System32\drivers\etc\hosts"
if (Test-Path $HostsPath) {
    if ((Get-Content $HostsPath -ErrorAction SilentlyContinue | Where-Object { $_ -match "^\s*[^#]" }) -match "virustotal" -or (Get-Content $HostsPath -ErrorAction SilentlyContinue) -match "drweb") {
        Write-Host "  [!] Alert: Anti-malware websites blockade found in HOSTS!" -ForegroundColor Red; $TotalThreats++; $ThreatList += "HOSTS File Anti-Virus Blockade"
    }
}
Write-Host "`n Checking Desktop shortcuts for adware links..." -ForegroundColor Cyan; $WshShell = New-Object -ComObject WScript.Shell
Get-ChildItem -Path "$HOME\Desktop" -Filter "*.lnk" -ErrorAction SilentlyContinue | ForEach-Object {
    $Shortcut = $WshShell.CreateShortcut($_.FullName); if ($Shortcut.TargetPath -match "http" -or $Shortcut.Arguments -match "http") {
        Write-Host "  [!] Adware link found in shortcut: '$($_.Name)'" -ForegroundColor Red; $TotalThreats++; $ThreatList += "Adware Desktop Shortcut: $($_.Name)"
    }
}
...
$CpuCounters.CounterSamples | Where-Object { $_.CookedValue -gt 50 -and $_.InstanceName -notlike "_total" -and $_.InstanceName -notlike "idle" } | ForEach-Object {
    ...
    $TotalThreats++; $ThreatList += "High Process CPU Load: $($_.InstanceName) ($Rounding%)"
}
Write-Host "`n Auditing active process signatures..." -ForegroundColor Cyan; $MinerSigs = @("xmrig", "minerd", "cpuminer", "ethminer", "stratum")
foreach ($Sig in $MinerSigs) {
    if (Get-Process -Name "*$Sig*" -ErrorAction SilentlyContinue) {
        Write-Host "  [!] Cryptominer process active signature found: $Sig" -ForegroundColor Red; $TotalThreats++; $ThreatList += "Active Cryptominer Signature: $Sig"
    }
}
if ($ScanMode -eq "2") {
